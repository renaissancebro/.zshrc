...
-- LSP document highlighting is now enabled (highlights same words when cursor is on them)
...
vim.keymap.set('n', '<leader>t', function()
  ...
  vim.defer_fn(function()
    ...
    vim.api.nvim_feedkeys('auto_activate_venv\r', 'n', false) -- run activation function
  end, 100)
...
-- Force disable git blame if it gets enabled accidentally
vim.keymap.set('n', '<leader>tb', function()
  pcall(function()
    require('gitsigns').toggle_current_line_blame()
    require('gitsigns').toggle_current_line_blame()  -- Force it off
  end)
  print("Git blame disabled")
end, { desc = 'Force disable git blame' })
...
require('lazy').setup({
  ...
  { -- Adds git related signs to the gutter, as well as utilities for managing changes
    ...
    opts = {
      ...
      current_line_blame = false, -- Disabled - was showing distracting "edited x minutes ago"
      ...
      on_attach = function(bufnr)
        ...
        -- Removed toggle_current_line_blame to prevent accidental activation
  ...
  { -- Useful plugin to show you pending keybinds.
    ...
    opts = {
      ...
      spec = {
        ...
        { '<leader>r', group = '[R]un/REPL' },
        { '<leader>m', group = '[M]olten/Jupyter' },
        { '<leader>n', group = '[N]eotest' },
        { '<leader>r', desc = '[R]un current file (with venv)' },
        { '<leader>py', desc = '[P]ython interpreter info' },
        { '<leader>sy', desc = '[S]yntax refresh' },
...

  {
    ...
    config = function()
      ...
      -- TypeScript LSP (fixed deprecation warning)
